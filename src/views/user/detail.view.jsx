            ...
            <div className="card">
              <div className="card-body text-center">
                <div className="mb-4">
                  ...
                  <h4>{'{{vueBind.model.userDetail.firstname}}'} {'{{vueBind.model.userDetail.lastname}}'}</h4>
                  <p className="text-muted mb-0">{'{{vueBind.model.userDetail.role.title}}'}</p>
                </div>
                <p className="mt-4 card-text"></p>
                <button className="btn btn-info btn-sm mt-3 mb-4">{'{{vueBind.model.userDetail.status.title}}'}</button>
                <div className="border-top pt-3">
                  <div className="row">
                    <div className="col-4">
                      <h6>{`{{vueBind.model.userDetail.last_login || '-'}}`}</h6>
                      <p>{this.props.data.locale.DETAIL_LAST_LOGIN_LABEL}</p>
                    </div>
                    <div className="col-4">
                      <h6>{`{{vueBind.model.userDetail.last_time_on_app || '-'}}`}</h6>
                      <p>{this.props.data.locale.DETAIL_LAST_TIME_LABEL}</p>
                    </div>
                    <div className="col-4">
                      <h6>{`{{vueBind.model.userDetail.phone || '-'}}`}</h6>
