class EventConsumerManager {
  ...
  async #connectToBroker () {
    ...
    this._consumer = this._websocketClientModule.connect(this._config.SERVICES.BROKER.URI, {
      reconnect: true
    })
  ...
  #registerEvents () {
    this._consumer.on('connect', (data) => {
      this._console.success(`connected to broker: ${this._config.SERVICES.BROKER.URI}`, { namespace: this._namespace })
    ...
    this._consumer.on('disconnect', () => {
      this._console.success(`disconnected from broker: ${this._config.SERVICES.BROKER.URI}`, { namespace: this._namespace })
    })
  }
  ...
  #subscribeTopics ({ consumer }) {
    ...
    consumer.join(this._eventSystemDefinition.broker.topics.map(topic => topic.name))
  }
