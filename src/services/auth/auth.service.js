class AuthService {
  ...
  async authenticateUser(data, user) {
    ...
    if (!authenticationResult) {
      return this._utilities.io.response.error(
        'Wrong password. Try again or click Forgot password to reset it. After 3 failed attempts your account will be blocked by 24 hours.',
      );
    }
    ...
    try {
      ...
      if (this._dependencies.config.MAX_HOURS_TOKEN_VALID <= hours) {
        return this._utilities.io.response.error(
          this._utilities.encoder.crypto.cypherObject(
            this._apiManagerService.key,
            'Token is outdated, please try requesting another email.',
          ),
        );
      }
      ...
      if (!decipheredToken || !decipheredToken.email) {
        return this._utilities.io.response.error(
          this._utilities.encoder.crypto.cypherObject(
            this._apiManagerService.key,
            'Token is not valid, please try requesting another email.',
          ),
        );
      }
  ...
  async validateAccountChatbot(data) {
    if (!data || !data.chat || !data.chat.user) {
      return this._utilities.io.response.error(
        'Is not possible validate account, please provide at least a phone number',
      );
    }
    const userService = new this._services.UserService(this._dependencies);
    const userResult = await userService.getByIdentity({
